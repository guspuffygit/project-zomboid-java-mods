...
function AtfCasinoBlackjackWindow:createChildren()
    ...
    local x = PAD
    self.sitBtn =
        self:addButton(x, y, BTN_W, txt("IGUI_AtfCasino_Blackjack_Sit", "Sit"), self.onSit)
    x = x + BTN_W + BTN_GAP
    self.leaveBtn =
        self:addButton(x, y, BTN_W, txt("IGUI_AtfCasino_Blackjack_Leave", "Get Up"), self.onLeave)
    x = x + BTN_W + GROUP_GAP
    ...
    x = x + ENTRY_W + BTN_GAP
    self.minBtn =
        self:addButton(x, y, QUICK_W, txt("IGUI_AtfCasino_Blackjack_Min", "Min"), self.onMin)
    x = x + QUICK_W + BTN_GAP
    self.maxBtn =
        self:addButton(x, y, QUICK_W, txt("IGUI_AtfCasino_Blackjack_Max", "Max"), self.onMax)
    x = x + QUICK_W + BTN_GAP
    self.betBtn =
    ...
    x = x + BTN_W + BTN_GAP
    self.standBtn =
        self:addButton(x, y, BTN_W, txt("IGUI_AtfCasino_Blackjack_Stand", "Stand"), self.onStand)
    x = x + BTN_W + BTN_GAP
    self.doubleBtn =
    ...
    self.closeBtn = self:addButton(
        ...
        y,
        BTN_W,
        txt("IGUI_AtfCasino_Blackjack_Close", "Close"),
        self.close
    )
...
function AtfCasinoBlackjackWindow:applyState(state)
    ...
    if AtfCasinoSounds then
        AtfCasinoSounds.blackjack(oldState, state)
    end
...
function AtfCasinoBlackjackWindow:render()
    ...
    elseif s.phase == "BETTING" then
        ...
        if (s.secondsLeft or 0) > 0 then
            status = txt(
                "IGUI_AtfCasino_Blackjack_BetsClosing",
                "Place your bets - dealing in %1s",
                tostring(secs)
            )
        else
    ...
    elseif s.phase == "PLAYING" then
        if s.canAct then
            status = txt(
                "IGUI_AtfCasino_Blackjack_YourTurn",
                "Your turn - %1s",
            ...
            status = txt(
                "IGUI_AtfCasino_Blackjack_Waiting",
                "Seat %1 is playing",
                tostring(s.currentSeat or 0)
            )
        end
    else
        status = txt(
            "IGUI_AtfCasino_Blackjack_Settle",
            "Round over - next round in %1s",
            tostring(self:secondsLeft())
        )
    end
    ...
    if s then
        local lim = txt(
            "IGUI_AtfCasino_Blackjack_Limits",
            "Bets %1 - %2",
            fmt(s.minBet or 0),
            fmt(s.maxBet or 0)
        )
    ...
    if command == "guardShot" then
        ...
        if player then
            ...
            if args and args.reason == "godmode" then
                text = txt(
                    "IGUI_AtfCasino_GuardShotGodMode",
                    "The Creepy Spiffos don't serve god-mode admins!"
                )
...
local function onFillWorldObjectContextMenu(playerNum, context, worldobjects, test)
    ...
    local option = context:addOption(
        txt(
            "IGUI_AtfCasino_Blackjack_ContextOption",
            "Play Blackjack with the Creepy Spiffo Dealer"
        ),
        worldobjects,

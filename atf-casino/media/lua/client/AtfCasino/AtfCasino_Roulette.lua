...
function AtfCasinoRouletteWindow:createChildren()
    ...
    x = x + BTN_W + BTN_GAP
    self.leaveBtn =
        self:addButton(x, y, BTN_W, txt("IGUI_AtfCasino_Roulette_Leave", "Get Up"), self.onLeave)
    x = x + BTN_W + GROUP_GAP
    ...
    x = x + ENTRY_W + BTN_GAP
    self.minBtn =
        self:addButton(x, y, QUICK_W, txt("IGUI_AtfCasino_Roulette_Min", "Min"), self.onMin)
    x = x + QUICK_W + BTN_GAP
    self.maxBtn =
        self:addButton(x, y, QUICK_W, txt("IGUI_AtfCasino_Roulette_Max", "Max"), self.onMax)
    x = x + QUICK_W + GROUP_GAP
    self.clearBtn = self:addButton(
        x,
        y,
        BTN_W + 20,
        txt("IGUI_AtfCasino_Roulette_Clear", "Clear Bets"),
        self.onClear
    )
    ...
    self.closeBtn = self:addButton(
        ...
        y,
        BTN_W,
        txt("IGUI_AtfCasino_Roulette_Close", "Close"),
        self.close
    )
...
function AtfCasinoRouletteWindow:close()
    if AtfCasinoSounds then
        AtfCasinoSounds.stopRouletteSpin(self)
    end
...
function AtfCasinoRouletteWindow:applyState(state)
    ...
    if AtfCasinoSounds then
        AtfCasinoSounds.roulette(self, oldState, state)
    end
...
function AtfCasinoRouletteWindow:drawBoard(s)
    ...
    self:drawText(
        hint,
        bx + BOARD_PAD,
        by + BOARD_PAD + BOARD_ROWS * CELL_H + 6,
        0.85,
        0.85,
        0.85,
        1,
...
function AtfCasinoRouletteWindow:drawWheel(s)
    ...
    self:drawText(
        txt("IGUI_AtfCasino_Roulette_Wheel", "Wheel"),
        wx + 12,
        wy + 8,
        1,
        1,
    ...
    self:drawText(
        txt("IGUI_AtfCasino_Roulette_History", "Last numbers"),
        wx + 12,
        hy,
        0.7,
        0.7,
...
function AtfCasinoRouletteWindow:render()
    ...
    if s then
        local lim = txt(
            "IGUI_AtfCasino_Roulette_Limits",
            "Chips %1 - %2",
            fmt(s.minBet or 0),
            fmt(s.maxBet or 0)
        )
    ...
    for i = 1, maxSeats do
        ...
        if seat then
            ...
            if (seat.staked or 0) > 0 then
                stakeLine =
                    txt("IGUI_AtfCasino_Roulette_SeatStaked", "On the board: %1", fmt(seat.staked))
            else
            ...
            if settled and (seat.staked or 0) > 0 then
                if (seat.payout or 0) > 0 then
                    ...
                    self:drawText(
                        txt("IGUI_AtfCasino_Roulette_Lost", "LOST"),
                        tx,
                        ty,
                        1,
                        0.4,
            ...
            elseif seat.leaving then
                self:drawText(
                    txt("IGUI_AtfCasino_Roulette_Leaving", "leaving"),
                    tx,
                    ty,
                    0.7,
                    0.7,
...
local function onFillWorldObjectContextMenu(playerNum, context, worldobjects, test)
    ...
    local option = context:addOption(
        txt(
            "IGUI_AtfCasino_Roulette_ContextOption",
            "Play Roulette with the Creepy Spiffo Croupier"
        ),
        worldobjects,

...
local SEAT_H = SEAT_PAD + HGT_MEDIUM + HGT_SMALL + HGT_SMALL + 8 + CARD_H + 8 + HGT_MEDIUM + HGT_SMALL + SEAT_PAD
...
function AtfCasinoHoldemWindow:createChildren()
	...
	x = x + BTN_W + BTN_GAP
	self.leaveBtn = self:addButton(x, y, BTN_W, txt("IGUI_AtfCasino_Holdem_Leave", "Cash Out"), self.onLeave)
	x = x + BTN_W + GROUP_GAP
	self.buyEntry = self:addEntry(x, y)
	x = x + ENTRY_W + BTN_GAP
	self.buyMinBtn = self:addButton(x, y, QUICK_W, txt("IGUI_AtfCasino_Holdem_Min", "Min"), self.onBuyMin)
	x = x + QUICK_W + BTN_GAP
	self.buyMaxBtn = self:addButton(x, y, QUICK_W, txt("IGUI_AtfCasino_Holdem_Max", "Max"), self.onBuyMax)
	x = x + QUICK_W + BTN_GAP
	self.buyBtn = self:addButton(x, y, BTN_W, txt("IGUI_AtfCasino_Holdem_BuyIn", "Buy In"), self.onBuyIn)
	self.closeBtn =
		self:addButton(self.width - BTN_W - PAD, y, BTN_W, txt("IGUI_AtfCasino_Holdem_Close", "Close"), self.close)
	...
	y = self.actionsY
	x = PAD
	self.foldBtn = self:addButton(x, y, BTN_W, txt("IGUI_AtfCasino_Holdem_Fold", "Fold"), self.onFold)
	x = x + BTN_W + BTN_GAP
	self.callBtn = self:addButton(x, y, CALL_W, txt("IGUI_AtfCasino_Holdem_Check", "Check"), self.onCheckOrCall)
	x = x + CALL_W + GROUP_GAP
	self.raiseEntry = self:addEntry(x, y)
	x = x + ENTRY_W + BTN_GAP
	self.raiseMinBtn = self:addButton(x, y, QUICK_W, txt("IGUI_AtfCasino_Holdem_Min", "Min"), self.onRaiseMin)
	x = x + QUICK_W + BTN_GAP
	self.raisePotBtn = self:addButton(x, y, QUICK_W, txt("IGUI_AtfCasino_Holdem_Pot", "Pot"), self.onRaisePot)
	x = x + QUICK_W + BTN_GAP
	self.raiseBtn = self:addButton(x, y, BTN_W, txt("IGUI_AtfCasino_Holdem_Raise", "Raise"), self.onRaise)
	x = x + BTN_W + BTN_GAP
	self.allInBtn = self:addButton(x, y, BTN_W, txt("IGUI_AtfCasino_Holdem_AllIn", "All-in"), self.onAllIn)
	self:refreshButtons()
end
...
function AtfCasinoHoldemWindow:onRaise()
	...
	if amount <= 0 then
		self:showError(txt("IGUI_AtfCasino_Holdem_Err_EnterRaise", "Enter the amount to raise to first"))
		return
	end
...
function AtfCasinoHoldemWindow:refreshButtons()
	...
	if s and s.canCheck then
		...
		self.callBtn:setTitle(txt("IGUI_AtfCasino_Holdem_Call", "Call %1", fmt((s and s.callAmount) or 0)))
	end
...
function AtfCasinoHoldemWindow:statusLine()
	...
	return txt("IGUI_AtfCasino_Holdem_Waiting", "Waiting for %1 - %2s", actingName(s), secs), 1, 1, 1
end
	...
	for i = 1, maxSeats do
		...
		if seat then
			...
			if (seat.won or 0) > 0 then
				label, r, g, b = txt("IGUI_AtfCasino_Holdem_Won", "WON +%1", fmt(seat.won)), 0.4, 1, 0.4
			...
			elseif s and s.phase ~= "WAITING" and s.phase ~= "SHOWDOWN" and not seat.inHand then
				label, r, g, b = txt("IGUI_AtfCasino_Holdem_SittingOut", "sitting out"), 0.6, 0.6, 0.6
			end
			if label then
...
local function onFillWorldObjectContextMenu(playerNum, context, worldobjects, test)
	...
	local option = context:addOption(
		txt("IGUI_AtfCasino_Holdem_ContextOption", "Play Texas Hold'em with the Creepy Spiffo Poker Dealer"),
		worldobjects,
	...
	if math.abs(player:getX() - DEALER.x) > 8 or math.abs(player:getY() - DEALER.y) > 8 then
		...
		tip.description = txt("IGUI_AtfCasino_Holdem_TooFarTip", "Walk up to the poker dealer first")
		option.toolTip = tip
	end
end

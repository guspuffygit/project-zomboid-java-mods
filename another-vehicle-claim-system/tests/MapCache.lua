...
WorldMapOptions = {
    ...
    synchUI = function() end,
}
UIFont = { Small = 1 }
PZMath = { floor = math.floor }
...
local map = {
    ...
    javaObject = {
        ...
        DrawTextCentre = function()
            labels = labels + 1
        end,
    },
    mapAPI = {
        worldToUIX = function(_, x)
            return x
        end,
        worldToUIY = function(_, x, y)
            return y
        end,
    },
}
...
check(
    translations == 1 and measures == 1 and labels == 1000,
    "Static map reuses metadata and label widths without duplicate group markers"
)
AVCS.dbByVehicleSQLID[1].LastLocationX = 1000
AVCS.cacheRevision = 2
ISWorldMap.render(map)
check(
    labels == 1000 and translations == 2,
    "Claim revision immediately moves and culls offscreen marker"
)
AVCS.dbByVehicleSQLID[1].LastLocationX = 110
AVCS.cacheRevision = 3
ISWorldMap.render(map)
check(labels == 1001, "Label crossing viewport edge remains visible")
handlers.SyncFaction()
ISWorldMap.render(map)
check(translations == 4, "Faction update invalidates metadata immediately")
handlers.OnSafehousesChanged()
ISWorldMap.render(map)

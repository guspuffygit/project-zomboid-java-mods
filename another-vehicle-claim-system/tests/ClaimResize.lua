...
getCore = function()
    return {
        ...
        getOptionFontSizeReal = function()
            return 1
        end,
    }
end
...
function UI:setWidth(v)
    ...
    for _, c in ipairs(self.children or {}) do
        if c.anchorRight then
            if c.anchorLeft then
                c:setWidth(c.width + delta)
            else
                c:setX(c.x + delta)
            end
        end
...
function UI:setHeight(v)
    ...
    self.height = v
    for _, c in ipairs(self.children or {}) do
        if c.anchorBottom then
            if c.anchorTop then
                c:setHeight(c.height + delta)
            else
                c:setY(c.y + delta)
            end
        end
...
function UI:setCapture(v)
    self.capture = v
end
function UI:getMouseX()
    return mx - self.x - (self.parent and self.parent.x or 0)
end
function UI:getMouseY()
    return my - self.y - (self.parent and self.parent.y or 0)
end
ISPanel = UI:derive()
ISButton = UI:derive()
ISLabel = UI:derive()
...
ISLayoutManager = {
    RegisterWindow = function(_, funcs)
        savedFunctions = funcs
    end,
}
local dialog
ISTextBox = {
    new = function(_, x, y, w, h, text, entry, target, callback)
        dialog = {
...
p:createChildren()
check(
    p.resizable and p.resizeWidget:getIsVisible() and p.resizeWidget2:getIsVisible(),
    "Native resize handles enabled after AVCS constructor disables them"
)
...
check(
    hit(p.avcsSizeButton.x + 5, p.avcsSizeButton.y + 5) == p.avcsSizeButton,
    "Size button is clickable"
)
p.avcsSizeButton.onclick(p.avcsSizeButton.target, p.avcsSizeButton)
dialog.entry.getText = function()
    return "1000x620"
end
...
savedFunctions.RestoreLayout(
    p,
    "VehicleClaims",
    { x = "20", y = "20", width = "1150", height = "680" }
)
...
check(
    p.lblVehicleLocation.x == p.lblVehicleOwner.x
        and p.lblVehicleLocation.y > p.lblVehicleExpireInfo.y,
    "Narrow window stacks info columns instead of overflowing"
)
check(
    p.vehiclePreview.height > 0 and p.vehiclePreview.y + p.vehiclePreview.height < p.resizeWidget.y,
    "Compact preview stays above drag controls"
)
check(
    p.resizeWidget.width >= 24 and p.resizeWidget.height >= 24,
    "Corner has a usable mouse target"
)
check(
    p.children[#p.children - 1] == p.resizeWidget2 or p.children[#p.children] == p.resizeWidget2,

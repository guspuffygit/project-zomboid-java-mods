...
getPlayer = function()
    return {
        getAccessLevel = function()
            return admin and "admin" or "none"
        end,
    }
end
isClient = function()
...
function UI:new(x, y, w, h)
    return setmetatable({
        ...
        anchorTop = true,
        anchorLeft = true,
        javaObject = { fromLua1 = function() end, fromLua2 = function() end },
    }, self)
end
...
for _, axis in ipairs({ "Top", "Bottom", "Left", "Right" }) do
    UI["setAnchor" .. axis] = function(self, v)
        self["anchor" .. axis] = v
    end
end
function UI:setX(v)
...
function UI:setWidth(v)
    local delta = v - self.width
    self.width = v
    for _, child in ipairs(self.children) do
        if child.anchorRight then
            if child.anchorLeft then
                child:setWidth(child.width + delta)
            else
...
function UI:setHeight(v)
    local delta = v - self.height
    self.height = v
    for _, child in ipairs(self.children) do
        if child.anchorBottom then
            if child.anchorTop then
                child:setHeight(child.height + delta)
            else
...
ISCollapsableWindow = UI:derive()
ISPanel = UI:derive()
ISButton = UI:derive()
ISScrollingListBox = UI:derive()
ISUI3DScene = UI:derive()
ISLabel = UI:derive()
function ISLabel:new(x, y, h, text)
    return UI.new(self, x, y, 100, h)
end
sizingAPI = {
    fit = function(p, w, h)
        p:setWidth(w)
    ...
    resizable = function(p, key, w, h, layout)
        p.layout = layout
        p:layout()
    end,
}
...
for _, f in ipairs({ 14, 28, 40 }) do
    ...
    for _, membership in ipairs({
        ...
        for _, isAdmin in ipairs({ false, true }) do
            ...
            AVCS = {
                UI = {
                    AdminManagerMain = { createChildren = function() end },
                    UserPermissionPanel = { createChildren = function() end },
                },
                getUIFontScale = function()
                    return scale
                end,
            }
            ...
            for _, size in ipairs({
                ...
                local buttons = {}
                for _, b in ipairs(panel.tabButtons) do
                    buttons[#buttons + 1] = b
                end
                buttons[#buttons + 1] = panel.btnModify
                buttons[#buttons + 1] = panel.btnUnclaim
                if panel.btnTeleport then
                    buttons[#buttons + 1] = panel.btnTeleport
                end
                local lastBottom = panel.listVehicles.y
                for _, b in ipairs(buttons) do
                    check(
                        b.y >= lastBottom and b.y + b.height <= panel.height - 18,
                        "Toolbar overlaps or escapes window"
                    )
                    check(
                        b.width == b.height and b.x + b.width <= panel.leftPaneHolder.width,
                        "Toolbar squares fit left pane"
                    )
                    check(
                        b.anchorTop and not b.anchorBottom,
                        "Tabs must not stretch on native resize"
                    )
                    lastBottom = b.y + b.height
                end
                local preview = panel.vehiclePreview
                check(
                    preview.anchorTop and not preview.anchorBottom,
                    "Preview must not drift vertically"
                )
                check(
                    preview.y > panel.lblVehicleExpireInfo.y + font,
                    "Preview leaves room for info"
                )
                check(
                    preview.height > 0 and preview.y + preview.height <= panel.height - 18,
                    "Preview fits vertically"
                )
                check(preview.x + preview.width <= panel.width - 8, "Preview fits horizontally")
                check(
                    panel.listVehicles.y + panel.listVehicles.height <= panel.height - 18,
                    "List clears resize handle"
                )
                for _, name in ipairs(labels) do
                    check(
                        panel[name].y >= 28 and panel[name].y + font < preview.y,
                        "Info stays in header"
                    )
                end
                if size[1] == 1500 then
                    check(
                        panel.listVehicles.width == originalListWidth,
                        "Grow restores original list width"
                    )
                end
                ...
                check(
                    preview.y == y and buttons[1].height == h,
                    "Deferred anchor propagation changes geometry"
                )
                sizingAPI.fit(panel, unpack(size))
            end
        end

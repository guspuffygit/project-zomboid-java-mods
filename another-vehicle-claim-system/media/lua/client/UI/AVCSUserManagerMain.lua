...
function AVCS.UI.UserManagerMain:setVehiclePreview(vehicleID)
    ...
    if vehicleID == nil then
        ...
        self.vehiclePreview.javaObject:fromLua2(
            "setVehicleScript",
            "previewVeh",
            AVCS.dbByVehicleSQLID[vehicleID].CarModel
        )
        ...
        self.lblVehicleExpireInfo:setName(
            os.date(
                "%d-%b-%y, %H:%M:%S",
                (
                    AVCS.dbByPlayerID[AVCS.dbByVehicleSQLID[vehicleID].OwnerPlayerID].LastKnownLogonTime
                    + (SandboxVars.AVCS.ClaimTimeout * 60 * 60)
                )
            )
        )
        self.lblVehicleLocationInfo:setName(
            AVCS.dbByVehicleSQLID[vehicleID].LastLocationX
                .. ", "
                .. AVCS.dbByVehicleSQLID[vehicleID].LastLocationY
        )
        self.lblVehicleLastLocationUpdateInfo:setName(
            os.date(
                "%d-%b-%y, %H:%M:%S",
...
function AVCS.UI.UserManagerMain:btnUnclaim_onConfirmClick(btn, _, _)
    ...
    sendClientCommand(
        ...
        "AVCS",
        "unclaimVehicle",
        { self.listVehicles.items[self.listVehicles.selected].item }
    )
...
function AVCS.UI.UserManagerMain:btnUnclaim_onClick(btn)
    ...
    self.modDialog = ISModalDialog:new(
        ...
        true,
        self,
        AVCS.UI.UserManagerMain.btnUnclaim_onConfirmClick,
...
function AVCS.UI.UserManagerMain:updateListVehicles()
    ...
    elseif prevTabBtn.internal == "tabSafehouse" then
        ...
        if safehouseObj then
            ...
            for i = 0, tempPlayers:size() - 1 do
                if
                    tempPlayers:get(i) ~= getPlayer():getUsername()
                    and AVCS.dbByPlayerID[tempPlayers:get(i)]
                then
    ...
    elseif prevTabBtn.internal == "tabFaction" then
        ...
        if factionObj then
            ...
            -- Dirty codings
            if
                factionObj:getOwner() ~= getPlayer():getUsername()
                and AVCS.dbByPlayerID[factionObj:getOwner()]
            then
...
function AVCS.UI.UserManagerMain:addTabButtons(btnName, bgImage, x, y)
    ...
    self.tabButtons[i] = ISButton:new(
        x,
        y,
        self.tabBtnSize,
        self.tabBtnSize,
        "",
        self,
        AVCS.UI.UserManagerMain.tabBtn_onClick
    )
...
function AVCS.UI.UserManagerMain:createChildren()
    ...
    self.leftPaneHolder = ISPanel:new(
        0,
        padTop,
        leftPaneHolderWidth,
        self.height - 1 - getTextManager():getFontHeight(UIFont.NewSmall),
        "",
        self,
        AVCS.UI.UserManagerMain.btnUnclaim_onClick
    )
    ...
    self.listVehicles = ISScrollingListBox:new(
        leftPaneHolderWidth + 1,
        padTop + 1,
        listVehiclesWidth,
        self.height - padTop - 1
    )
    ...
    self.vehiclePreview = ISUI3DScene:new(
        leftPaneHolderWidth + 1 + listVehiclesWidth + 1,
        padTop,
        self.width - leftPaneHolderWidth - 1 - listVehiclesWidth - 1,
        self.height - padTop
    )
    ...
    self.lblVehicleOwner = ISLabel:new(
        leftPaneHolderWidth + 1 + listVehiclesWidth + 1 + padBetween,
        padTop + 1,
        getTextManager():getFontHeight(UIFont.NewSmall),
        getText("IGUI_AVCS_User_Manager_lblVehicleOwner"),
        1,
        1,
        1,
        1,
    ...
    self.lblVehicleOwnerInfo = ISLabel:new(
        leftPaneHolderWidth + 1 + listVehiclesWidth + 1 + padBetween,
        padTop + 1 + ((getTextManager():getFontHeight(UIFont.NewSmall)) * 1) + (1 * 1),
        getTextManager():getFontHeight(UIFont.NewSmall),
        "",
        0.2,
        1,
    ...
    self.lblVehicleExpire = ISLabel:new(
        leftPaneHolderWidth + 1 + listVehiclesWidth + 1 + padBetween,
        padTop + 1 + ((getTextManager():getFontHeight(UIFont.NewSmall)) * 2) + (1 * 2),
        getTextManager():getFontHeight(UIFont.NewSmall),
        getText("IGUI_AVCS_User_Manager_lblVehicleExpire"),
        1,
        1,
        1,
        1,
    ...
    self.lblVehicleExpireInfo = ISLabel:new(
        leftPaneHolderWidth + 1 + listVehiclesWidth + 1 + padBetween,
        padTop + 1 + ((getTextManager():getFontHeight(UIFont.NewSmall)) * 3) + (1 * 3),
        getTextManager():getFontHeight(UIFont.NewSmall),
        "",
        0.2,
        1,
    ...
    local rightTextPad = math.floor(235 * AVCS.getUIFontScale())
        - 8 * (getCore():getOptionFontSize() - 1)
    self.lblVehicleLocation = ISLabel:new(
        leftPaneHolderWidth + 1 + listVehiclesWidth + 1 + padBetween + rightTextPad,
        padTop + 1,
        getTextManager():getFontHeight(UIFont.NewSmall),
        getText("IGUI_AVCS_User_Manager_lblVehicleLocation"),
        1,
        1,
        1,
        1,
    ...
    self.lblVehicleLocationInfo = ISLabel:new(
        leftPaneHolderWidth + 1 + listVehiclesWidth + 1 + padBetween + rightTextPad,
        padTop + 1 + ((getTextManager():getFontHeight(UIFont.NewSmall)) * 1) + (1 * 1),
        getTextManager():getFontHeight(UIFont.NewSmall),
        "",
    ...
    self.lblVehicleLastLocationUpdate = ISLabel:new(
        leftPaneHolderWidth + 1 + listVehiclesWidth + 1 + padBetween + rightTextPad,
        padTop + 1 + ((getTextManager():getFontHeight(UIFont.NewSmall)) * 2) + (1 * 2),
        getTextManager():getFontHeight(UIFont.NewSmall),
        getText("IGUI_AVCS_User_Manager_lblVehicleLastLocationUpdate"),
        1,
        1,
        1,
        1,
    ...
    self.lblVehicleLastLocationUpdateInfo = ISLabel:new(
        leftPaneHolderWidth + 1 + listVehiclesWidth + 1 + padBetween + rightTextPad,
        padTop + 1 + ((getTextManager():getFontHeight(UIFont.NewSmall)) * 3) + (1 * 3),
        getTextManager():getFontHeight(UIFont.NewSmall),
        "",
    ...
    self:addTabButtons(
        ...
        getTextManager():getFontHeight(UIFont.NewSmall) + 1 + 5
    )
    ...
    if SandboxVars.AVCS.AllowSafehouse then
        if SafeHouse.hasSafehouse(getPlayer()) then
            ...
            self.tabButtons[#self.tabButtons]:setTooltip(
                getText("IGUI_AVCS_User_Manager_tabButton_Safehouse_Tooltip")
            )
        end
    end
    if SandboxVars.AVCS.AllowFaction then
        if Faction.getPlayerFaction(getPlayer()) then
            ...
            self.tabButtons[#self.tabButtons]:setTooltip(
                getText("IGUI_AVCS_User_Manager_tabButton_Faction_Tooltip")
            )
        end
    end
    ...
    self.btnModify = ISButton:new(
        ...
        "",
        self,
        AVCS.UI.UserManagerMain.btnModify_onClick
    )

...
function AVCS.UI.AdminManagerMain:listOnSelectionChange()
    ...
    then
        ...
        if
            Faction.getPlayerFaction(self.listData.items[self.listData.selected].item.OwnerPlayerID)
        then
...
function AVCS.UI.AdminManagerMain:drawData(y, item, alt)
    ...
    self:setStencilRect(clipX, clipY, clipX2 - clipX, clipY2 - clipY)
    self:drawText(
        item.item.carFullName,
        self.columns[3].size + xoffset,
        y + 4,
        1,
        1,
        1,
    ...
    self:setStencilRect(clipX, clipY, clipX2 - clipX, clipY2 - clipY)
    self:drawText(
        item.item.ClaimDateTime,
        self.columns[4].size + xoffset,
        y + 4,
        1,
        1,
        1,
...
function AVCS.UI.AdminManagerMain:filterStringCompare(mainText, compareText)
    return checkStringPattern(compareText)
        and string.match(string.lower(mainText), string.lower(compareText))
end
...
function AVCS.UI.AdminManagerMain:onFilterChange()
    ...
    for k, v in ipairs(self.parent.varData) do
        local add = true
        if
            not AVCS.UI.AdminManagerMain:filterStringCompare(
                v.OwnerPlayerID,
                self.parent.textFilterUsername:getInternalText()
            )
        then
            add = false
        end
        if add then
            if
                not AVCS.UI.AdminManagerMain:filterStringCompare(
                    v.carFullName,
                    self.parent.textFilterVehicleName:getInternalText()
                )
            then
                add = false
            end
        ...
        for ak, av in pairs(v) do
            if ak ~= "LastKnownLogonTime" then
                ...
                carFullName = getTextOrNull(
                    "IGUI_VehicleName"
                        .. string.sub(carFullName, index + 1, string.len(carFullName))
                )
                ...
                table.insert(tempTable, {
                    ...
                    ExpireOn = os.date(
                        "%d-%b-%y, %H:%M:%S",
                        (
                            AVCS.dbByPlayerID[k].LastKnownLogonTime
                            + (SandboxVars.AVCS.ClaimTimeout * 60 * 60)
                        )
                    ),
                    Location = AVCS.dbByVehicleSQLID[ak].LastLocationX
                        .. ","
                        .. AVCS.dbByVehicleSQLID[ak].LastLocationY,
                    ClaimDateTime = os.date(
                        "%d-%b-%y, %H:%M:%S",
                        AVCS.dbByVehicleSQLID[ak].ClaimDateTime
                    ),
                })
            end
        end
...
function AVCS.UI.AdminManagerMain:createChildren()
    ...
    self.listData = ISScrollingListBox:new(
        30 * AVCS.getUIFontScale() + 10,
        20 + padTop + getTextManager():getFontHeight(UIFont.NewSmall),
        self.width - 40 * AVCS.getUIFontScale() - 5 + 1 * AVCS.getUIFontScale(),
        360 * AVCS.getUIFontScale()
    )
    ...
    self.lblFilter = ISLabel:new(
        41 * AVCS.getUIFontScale(),
        self.listData:getBottom() + 10,
        getTextManager():getFontHeight(UIFont.NewLarge),
        getText("IGUI_AVCS_Admin_Manager_lblFilter"),
        1,
        1,
        1,
        1,
    ...
    self.listFilterHeader = ISScrollingListBox:new(
        40 * AVCS.getUIFontScale(),
        self.lblFilter:getBottom() + 24 * AVCS.getUIFontScale(),
        452 * AVCS.getUIFontScale(),
        0
    )
    ...
    self.listFilterHeader:addColumn(
        getText("IGUI_AVCS_Admin_Manager_listCarName"),
        150 * AVCS.getUIFontScale()
    ) -- 300 width, 46 characters max
    self:addChild(self.listFilterHeader)
    self.listFilterHeader:setFont(UIFont.NewSmall, 5)

    self.textFilterUsername = ISTextEntryBox:new(
        "",
        40 * AVCS.getUIFontScale(),
        self.listFilterHeader:getBottom(),
        151 * AVCS.getUIFontScale(),
        getTextManager():getFontHeight(UIFont.NewMedium)
    )
    ...
    self.textFilterVehicleName = ISTextEntryBox:new(
        "",
        190 * AVCS.getUIFontScale(),
        self.listFilterHeader:getBottom(),
        302 * AVCS.getUIFontScale(),
        getTextManager():getFontHeight(UIFont.NewMedium)
    )
    ...
    self.btnViewSafehouse = ISButton:new(
        6,
        10 + getTextManager():getFontHeight(UIFont.NewSmall),
        tabBtnSize,
        tabBtnSize,
        "",
        self,
    ...
    self.btnViewFaction = ISButton:new(
        6,
        tabBtnSize * 1
            + 10
    ...
    self.btnModifyPermissions = ISButton:new(
        6,
        tabBtnSize * 2
            + 10
            + getTextManager():getFontHeight(UIFont.NewSmall)
    ...
    self.btnModifyPermissions:setEnable(false)
    self.btnModifyPermissions:setTooltip(
        getText("IGUI_AVCS_Admin_Manager_btnModifyPermissions_Tooltip")
    )
    self:addChild(self.btnModifyPermissions)

    tempImage = getTexture("media/ui/avcs_delete.png")
    self.btnDelete = ISButton:new(
        6,
        tabBtnSize * 3
            + 10
            + getTextManager():getFontHeight(UIFont.NewSmall)
...
function AVCS.UI.AdminManagerMain:btnOnClick(btn)
    ...
    elseif btn.internal == "btnViewFaction" then
        local factionUI = ISFactionUI:new(
            getCore():getScreenWidth() / 2 - 250,
            getCore():getScreenHeight() / 2 - 225,
            500,
            450,
    ...
    elseif btn.internal == "btnModifyPermissions" then
        self.panelModify = AVCS.UI.UserPermissionPanel:new(
            (getCore():getScreenWidth() / 2) - (200 * AVCS.getUIFontScale() / 2),
            (getCore():getScreenHeight() / 2) - (300 * AVCS.getUIFontScale() / 2),
            200 * AVCS.getUIFontScale(),
            300 * AVCS.getUIFontScale(),
            self.listData.items[self.listData.selected].item.vehicleID
        )
    ...
    elseif btn.internal == "btnDelete" then
        ...
        self.modDialog = ISModalDialog:new(
            (getCore():getScreenWidth() / 2) - (250 / 2),
            (getCore():getScreenHeight() / 2) - (100 / 2),
            250,
            100,
            message,
            true,
            self,
            AVCS.UI.AdminManagerMain.btnUnclaim_onConfirmClick,
...
function AVCS.UI.AdminManagerMain:btnUnclaim_onConfirmClick(btn, _, _)
    ...
    self.removeUsernameVehicleID(
        self,
        self.listData.items[self.listData.selected].item.OwnerPlayerID,
        self.listData.items[self.listData.selected].item.vehicleID
    )

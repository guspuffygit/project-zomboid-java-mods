...
MultiplayerZoneEditorMode_ZoneMarker =
    MultiplayerZoneEditorMode:derive("MultiplayerZoneEditorMode_ZoneMarker")
...
function MultiplayerZoneEditorMode_ZoneMarker:createChildren()
    ...
    self.removeCatBtn = ISButton:new(
        self.categoryCombo:getRight() + 4,
        curY,
        20,
        BUTTON_HGT,
        "-",
    ...
    local nameLabel =
        ISLabel:new(UI_BORDER_SPACING, curY, rowH, "Name:", 1, 1, 1, 1, UIFont.Small, true)
    ...
    local gLabel =
        ISLabel:new(self.catREntry:getRight() + 4, curY, rowH, "G:", 1, 1, 1, 1, UIFont.Small, true)
    ...
    local bLabel =
        ISLabel:new(self.catGEntry:getRight() + 4, curY, rowH, "B:", 1, 1, 1, 1, UIFont.Small, true)
    ...
    local aLabel =
        ISLabel:new(self.catBEntry:getRight() + 4, curY, rowH, "A:", 1, 1, 1, 1, UIFont.Small, true)
    ...
    self.addCatBtn = ISButton:new(
        self.catAEntry:getRight() + UI_BORDER_SPACING,
        curY,
        50,
        rowH,
        "Add",
    ...
    -- Zone list
    local zoneLabel =
        ISLabel:new(UI_BORDER_SPACING, curY, BUTTON_HGT, "Zones:", 1, 1, 1, 1, UIFont.Small, true)
    ...
    -- Zone buttons
    self.addZoneBtn =
        ISButton:new(UI_BORDER_SPACING, curY, 80, BUTTON_HGT, "+ Add Zone", self, self.onAddZone)
    self:addChild(self.addZoneBtn)

    self.removeZoneBtn = ISButton:new(
        self.addZoneBtn:getRight() + 4,
        curY,
        90,
        BUTTON_HGT,
        "- Remove Zone",
...
function MultiplayerZoneEditorMode_ZoneMarker:prerender()
    ...
    self.removeZoneBtn:setEnable(
        hasCats and self.zoneList.selected > 0 and self.zoneList.selected <= #self.zoneList.items
    )
end
...
function MultiplayerZoneEditorMode_ZoneMarker:render()
    ...
    elseif self.drawState == "drawing" and self.drawStart and self.drawEnd then
        ...
        if cat then
            cr, cg, cb, ca = cat.r, cat.g, cat.b, cat.a
        end
        ...
        self.mapUI.javaObject:DrawTextureScaledColor(
            nil,
            PZMath.floor(rx),
            PZMath.floor(ry),
            rw,
...
function MultiplayerZoneEditorMode_ZoneMarker:refreshZoneList()
    ...
    if self.zoneList.selected > 0 and self.zoneList.selected <= #self.zoneList.items then
        prevRegion = self.zoneList.items[self.zoneList.selected].item
            and self.zoneList.items[self.zoneList.selected].item.region
    end
    ...
    if zones then
        for i, zone in ipairs(zones) do
            local label = zone.region
                .. "  ("
                .. PZMath.floor(zone.xStart)
                .. ","
                .. PZMath.floor(zone.yStart)
                .. " -> "
                .. PZMath.floor(zone.xEnd)
                .. ","
                .. PZMath.floor(zone.yEnd)
                .. ")"
...
function MultiplayerZoneEditorMode_ZoneMarker:getSelectedCategoryName()
    if self.categoryCombo:getOptionCount() == 0 then
        return nil
    end
...
function MultiplayerZoneEditorMode_ZoneMarker:onRemoveCategory()
    ...
    local modal = ISModalDialog:new(
        self.removeCatBtn:getRight() + 20,
        self.removeCatBtn:getY(),
        350,
        150,
        "Remove category '" .. name .. "'? All zones in this category will be deleted.",
        true,
        self,
        self.onConfirmRemoveCategory
    )
    modal:initialise()
...
function MultiplayerZoneEditorMode_ZoneMarker:onRemoveZone()
    ...
    local sel = self.zoneList.selected
    if sel <= 0 or sel > #self.zoneList.items then
        return
    end
    local zone = self.zoneList.items[sel].item
    if not zone then
    ...
    self:closeModal()
    local modal = ISModalDialog:new(
        self.removeZoneBtn:getRight() + 20,
        self.removeZoneBtn:getY(),
        350,
        150,
        "Remove zone '" .. zone.region .. "' from '" .. catName .. "'?",
        true,
        self,
        self.onConfirmRemoveZone
    )
    modal:initialise()
    modal:addToUIManager()
    modal:setAlwaysOnTop(true)

...
getUsers = function()
    return javaList(users)
end
AdminCore = { stats = {} }
dofile(client .. "OfflinePlayers.lua")
local sorted = AdminCore.sortedAccounts()
check(
    sorted[1].username == "online" and sorted[2].username == "new" and sorted[3].username == "old",
    "Online first, then most recent offline account"
)
...
ISAddSafeZoneUI = { instance = legacy() }
ISAddNonPvpZoneUI = {
    new = function()
        return legacy()
    end,
...
check(
    removed == 2 and ISAddSafeZoneUI.instance == nil and AdminCore.legacyNonPvp == nil,
    "Competing creators dismissed"
)
...
local zone = {
    ...
    dragging = true,
    heldWorldMenu = true,
    previousWorldMenu = false,
    releaseMouse = AdminCoreWorldSafezone.releaseMouse,
}
ISWorldObjectContextMenu = { disableWorldMenu = true }
AdminCoreWorldSafezone.finishDrag(zone)
check(
    not zone.dragging and not zone.drawing and not ISWorldObjectContextMenu.disableWorldMenu,
    "Release locks bounds and restores world menu"
)
local offline = AdminCore.sortedAccounts(nil, true)
check(
    #offline == 2 and offline[1].username == "new",
    "Offline picker excludes online users and keeps recent-first order"
)
...
getOnlinePlayers = function()
    return javaList(online)
end
local responses, calls = {}, {}
sendServerCommand = function(p, m, c, a)
    responses[#responses + 1] = { command = c, args = a }
end
AdminCoreBridge = {
    knownUser = function(_, name)
        return name == "admin" or name == "guest"
    end,
    lastSeen = function()
        return 123
    end,
    ...
    transferAndRemove = function(...)
        calls[#calls + 1] = "transfer"
        return "OK"
    end,
    canObserve = function()
        return caps.observe
...
check(
    #responses == 0 and #calls == 0,
    "Unprivileged clients cannot query data or change membership"
)
caps.ReadUserLog = true
...
now = now + 2000
S.tick()
check(persisted.admin.seconds == 4, "Session totals accumulate")
caps.observe = true
caps.follow = true
caps.TeleportToPlayer = true
...
check(
    type(returned) == "table" and returned.x == 1 and returned.y == 2 and returned.z == 0,
    "Expired observation returns to server-recorded origin"
)

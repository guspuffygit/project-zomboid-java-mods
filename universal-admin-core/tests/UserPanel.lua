...
local function button(x, y, w, h)
    return {
        ...
        setEnable = function(self, v)
            self.enable = v
        end,
    }
end
ISButton = {
    new = function(_, x, y, w, h, text, target, callback)
        local b = button(x, y, w, h)
        b.target = target
        b.callback = callback
        return b
    end,
...
local function panel(existing)
    local p = {
        height = 200,
        factionBtn = button(10, 40, 150, 20),
        ticketsBtn = button(10, 70, 150, 20),
        safehouseBtn = existing,
        player = {
    ...
    function p:addChild(child)
        self.children[#self.children + 1] = child
    end
    function p:setHeight(v)
        self.height = v
    end
    return p
end
...
check(
    p.safehouseBtn.internal == "SAFEHOUSEPANEL"
        and p.safehouseBtn.callback == ISUserPanelUI.onOptionMouseDown,
    "Native action retained"
)
check(p.ticketsBtn.y == 100 and p.height == 230, "Existing controls shift to make room")
ISUserPanelUI.updateButtons(p)
check(
    p.safehouseBtn.visible and not p.safehouseBtn.enable,
    "Missing membership keeps a visible disabled entry"
)
house = {}
ISUserPanelUI.updateButtons(p)
...
ISUserPanelUI.updateButtons(p)
check(
    not p.safehouseBtn.enable and nativeUpdates == 3,
    "Removal disables entry and native updates remain"
)
local existing = button(10, 70, 150, 20)
p = panel(existing)
ISUserPanelUI.create(p)

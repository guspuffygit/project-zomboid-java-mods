...
getUsers = function()
    error("Must not enumerate the server account directory")
end
local now = 10000
...
local requests = {}
sendClientCommand = function(m, c, args)
    requests[#requests + 1] = { command = c, args = args }
end
local list = {
    items = {},
    height = 300,
...
callbacks.OnTick()
check(
    #requests == 2 and requests[2].args.names[1] == "user101",
    "Scrolling requests the new viewport"
)
ISUsersList.instance = nil
now = 13000
callbacks.OnTick()
check(#requests == 2, "Closing list stops all data requests")
ISUsersList.instance = { datas = list }
allowed = false
now = 25000
callbacks.OnTick()
...
local empty = true
for _ in pairs(AdminCore.stats) do
    empty = false
    break
end
check(empty, "Disconnect clears cached player information")

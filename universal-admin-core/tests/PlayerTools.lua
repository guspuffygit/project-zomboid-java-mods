...
local row = {
    item = {
        getUsername = function()
            touched = touched + 1
            error("Hidden row was evaluated")
        end,
    },
}
for y = 1000, 100000 do
    ISUsersList.drawDatas(hidden, y, row, false)
end
check(touched == 0, "Off-screen rows do no account/faction/render work")
local T = AdminCore
local ownershipPickers = 0
T.replaceOwner = function()
    ownershipPickers = ownershipPickers + 1
end
local messages = {}
T.message = function(text)
...
local function window(w, h)
    ...
    for _, field in ipairs({ "Width", "Height", "X", "Y" }) do
        ...
        p["set" .. field] = function(self, v)
            self[key] = v
        end
        p["get" .. field] = function(self)
            return self[key]
        end
    end
    return p
...
local function menu()
    ...
    function m:addOption(name, target, callback, ...)
        ...
        self.options[self.numOptions] = o
...
local setRole = context:addOption("Set Role")
setRole.subOption = 42
local kick = context:addOption("Kick", ISUsersList, callback, {}, "Kick")
kick.notAvailable = true
kick.toolTip = { description = "Can't kick self" }
...
check(
    kick.notAvailable and kick.toolTip.description == "Can't kick self",
    "Preserve disabled and explanatory tooltip"
)
check(
    groups.Inventory.options[1] == inv and inv.onSelect == callback,
    "Preserve inventory callback"
)
...
Faction = {
    getPlayerFaction = function(name)
        if name == "guest" or name == "owner" then
            return faction
        end
    end,
}
local removedHouse, removedFaction, released, commands = 0, 0, 0, {}
sendSafehouseChangeMember = function()
    removedHouse = removedHouse + 1
end
sendFactionRemoveMember = function()
    removedFaction = removedFaction + 1
end
sendSafehouseRelease = function()
    released = released + 1
end
SendCommandToServer = function(cmd)
    commands[#commands + 1] = cmd
end
check(
    T.safehouse("guest") == house and T.safehouse("owner") == house and not T.safehouse("random"),
    "Membership includes owners and members only"
)
T.removeHouseMember("guest")
T.removeFactionMember("guest")
check(removedHouse == 0 and removedFaction == 0, "Unprivileged callbacks must not send packets")
local hidden = menu()
T.membershipMenu(hidden, "guest")
check(#hidden.options == 0, "Unprivileged users must not see management")
caps.CanSetupSafehouses = true
caps.FactionCheat = true
T.removeHouseMember("owner")
T.removeFactionMember("owner")
...
T.removeHouseMember("guest", "stale")
T.removeFactionMember("guest", "stale")
check(removedHouse == 0 and removedFaction == 0, "Stale confirmation cannot remove new membership")
T.removeHouseMember("guest", "house-a")
T.removeFactionMember("guest", "Test faction")
check(
    removedHouse == 1 and removedFaction == 1,
    "Authorized current member removal sends vanilla packets"
)
local visible = menu()
T.membershipMenu(visible, "guest")
local confirmation
T.confirm = function(text, action)
    confirmation = action
end
for _, o in ipairs(visible.options[1].sub.options) do
    if o.name == "Delete safehouse claim..." then
        o.onSelect()
    end
end
...
T.teleportCoordinates(100, 200, 0)
check(#commands == 0, "Teleport capability required")
caps.TeleportToCoordinates = true
T.teleportCoordinates(100.9, 200.9, 0)
...
input = "Base.DoesNotExist"
ISPlayerStatsManageInvUI.onAddItem(panel, button)
check(#commands == 2, "Reject unknown script item")
input = "Base.Hammer"
panel.playerUsername = 'bad"name'
ISPlayerStatsManageInvUI.onAddItem(panel, button)
...
local account = {
    ...
    getWarningPoints = function()
        return 0
    end,
    getSuspicionPoints = function()
        return 0
    end,
    getKicks = function()
        return 0
...
local list = {
    ...
    itemheight = 40,
    x = 0,
    parent = { uacColumns = { 0, 100, 200, 300, 400, 500, 600 } },
    getYScroll = function()
        return 0
    end,
}
...
check(
    list.uacDrawCache.guest == cached and factionReads == 1,
    "Visible rows reuse formatted text and faction display"
)
online = false
ISUsersList.drawDatas(list, 0, { item = account }, false)
check(
    list.uacDrawCache.guest.values[1][2] == "Offline",
    "Changed online state immediately invalidates row text"
)
T.stats.guest = { tracked = true, seconds = 3600 }
ISUsersList.drawDatas(list, 0, { item = account }, false)
check(
    list.uacDrawCache.guest.values[5][1] == "1.00 h",
    "Fresh playtime invalidates cached formatting"
)

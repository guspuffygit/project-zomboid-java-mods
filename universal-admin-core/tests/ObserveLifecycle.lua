...
AdminCoreBridge = {
    ...
    follow = function()
        if not powers or not targetOnline then
            return false
        end
        actor.x = 100
        actor.y = 200
    ...
    returnFromObserve = function(player, x, y, z)
        returns = returns + 1
        player.x = x
        player.y = y
        player.z = z
...
local function drain()
    for _ = 1, 10 do
        if #toServer == 0 and #toClient == 0 then
            return
        end
        while #toServer > 0 do
        ...
        while #toClient > 0 do
            ...
            for _, fn in ipairs(clientEvents.OnServerCommand.handlers) do
                fn(p.module, p.command, p.args)
            end
        end
    end
    ...
    if withClient ~= false then
        for _, fn in ipairs(clientEvents.OnTick.handlers) do
            if not pcall(fn) then
                clientErrors = clientErrors + 1
            end
        end
    end
    drain()
    AdminCoreServer.tick()
    drain()
end
local function start(name)
    sendClientCommand("UniversalAdminCore", "observeStart", { username = name or "guest" })
    drain()
end
start()
check(
    AdminCore.observing == "guest" and persisted.admin.returnPosition.x == 10,
    "Start records origin and acknowledges target"
)
for _ = 1, 120 do
    advance(1000)
    check(
        AdminCore.observing == "guest" and returns == 0,
        "Empty-payload heartbeats must keep observation alive beyond 12 seconds"
    )
end
check(
    heartbeatCount == 60 and cameraUpdates == 120 and camera == "guest",
    "Bounded heartbeats renew camera continuously"
)
local beforeStops = stopCount
start("guest-two")
advance(1000)
check(
    AdminCore.observing == "guest-two",
    "Switching target survives old-session stop acknowledgement"
)
...
AdminCore.visibleStatNames = function()
    error("Injected unrelated player-list failure")
end
for _ = 1, 15 do
    advance(1000)
end
check(
    clientErrors == 15 and AdminCore.observing == "guest-two" and camera == "guest-two",
    "Stats failures cannot starve heartbeat or camera updates"
)
AdminCore.visibleStatNames = visible
AdminCore.stopObserve()
drain()
check(
    AdminCore.observing == nil and camera == nil and AdminCore.observePanel == nil,
    "Stop clears camera and panel"
)
check(
    actor.x == 10 and actor.y == 20 and persisted.admin.returnPosition == nil,
    "Nil-payload stop returns immediately to origin"
)
...
advance(12000, false)
check(
    AdminCore.observing == nil and actor.x == 10,
    "Actual lost heartbeat still expires and returns"
)
start()
advance(2000)
powers = false
...
start()
advance(2000)
for _, fn in ipairs(clientEvents.OnKeyPressed.handlers) do
    fn(Keyboard.KEY_ESCAPE)
end
drain()
...
advance(2000)
online = {}
for _, fn in ipairs(clientEvents.OnDisconnect.handlers) do
    fn()
end
advance(2000, false)
check(
    camera == nil and persisted.admin.returnPosition.x == 10,
    "Disconnect clears camera and retains return point"
)
online = { actor }
advance(2000, false)

...
local function resolveEntryVehicle(entry)
	...
	local observedSql = NMVehicleHelpers and NMVehicleHelpers.getVehicleSqlIdString and NMVehicleHelpers.getVehicleSqlIdString(vehicle) or ""
...
local function applyPatch()
	if not (NMServerVehiclePowerTick and NMServerVehiclePowerTick.onEveryOneMinute
		and NMServerRegistryState and NMDeviceProfiles and NMDeviceState
		and NMVehicleHelpers and NMServerBatteryAuthority and NMRuntimeConfig and NMCore) then
		print("[ATFPatches] Talis New Music not loaded (or shape changed); vehicle power registry sweep skipped.")
		return false
	end
	...
	print("[ATFPatches] Talis New Music vehicle power tick now registry-driven (full sweep every " .. FULL_SWEEP_EVERY_N_MINUTES .. " minutes).")
	return true
end
